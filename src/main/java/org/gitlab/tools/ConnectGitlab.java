...
public class ConnectGitlab {
	...
	public static void main(String[] args) {
		 ...
		 */
		ConnectGitlab gtLab = new ConnectGitlab();
	...
	private void gitlabConnect(Properties prop) {
		...
		String[] groups = prop.getProperty("GROUPS").split(",");
		try {
			...
			Stream<GitlabGroup> filterAllGroup = allGroup.stream()
					.filter(p -> Arrays.asList(groups).contains(p.getName()));
			filterAllGroup.forEach(groupBody -> {
